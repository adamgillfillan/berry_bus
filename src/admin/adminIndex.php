<?php 
	...
	while($row=mysql_fetch_array($result)){
		...
		$formattedTime = date("M d g:i:s A", $realTime);
		
		$bus = array("timestamp"=>$formattedTime,"type"=>$row['Type'],"active"=>$row['Active']);		
		$buses[] = $bus;
	}
	...
	//if there are buses in the buses array then return the array else return there are no buses
	if(count($buses)>0){
